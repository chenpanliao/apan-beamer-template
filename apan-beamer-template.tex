...
\usepackage[
    ...
    CheckSingle=true, SlantFont, BoldFont]{xeCJK} 
\setCJKmainfont[Scale=1.2,BoldFeatures={FakeBold=2}]{cwTeX Q Hei Bold}
\setCJKmonofont[Scale=1.2,BoldFeatures={FakeBold=4},
    FakeStretch=1.015319]{cwTeX Q Yuan} 
...
請下載最新版字體以符合本文內容。}
\item[cwTeX-Q] {\url{http://code.google.com/p/cwtex-q-fonts/}}
\end{description}
\end{block}
...
\frametitle{本文件完整原始碼}
（以 Adobe Reader/Acrobat 開啟本PDF文件可取得內嵌的原始碼。）
\textattachfile[color=1 0 0]{\jobname{}.tex}{\jobname{}.tex}

{
  % 在 Verbatim 中關閉 xeCJK 的中英自動間隔並啟用原始標點符號設定
  \CJKsetecglue{}\punctstyle{plain} 
  \fvset{fontsize=\tiny, baselinestretch=1.1}
